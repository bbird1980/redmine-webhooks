...
class WebhookListener < Redmine::Hook::Listener
  def controller_issues_new_after_save(context = {})
    begin
      project = context[:project] || Project.find(context[:issue][:project_id])
      if project.module_enabled?("webhook")
        begin
          data = {
      ...
      Rails.logger.error "[WebhookListener::controller_issues_new_after_save] Exception: #{e.message}"
    end
  end
